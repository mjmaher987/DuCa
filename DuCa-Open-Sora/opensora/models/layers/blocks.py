...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cache_dic=None, current=None) -> torch.Tensor:
        B, N, C = x.shape
        ...
        if cache_dic['cache_type'] == 'kv-norm':
            if current['flag'] == -1:
                ...
                #cache_dic['cache'][current['flag']][current['layer']]['k_norm'] = torch.norm(
                #    rearrange(k, "(B S) H T C -> B H (T S) C", H=self.num_heads ,T=cache_dic['T'], S=cache_dic['S']), dim=-1, p=2)

                cache_dic['cache'][current['flag']][current['layer']]['v_norm'] = torch.norm(
                    rearrange(v, "(B S) H T C -> B H (T S) C", H=self.num_heads ,T=cache_dic['T'], S=cache_dic['S']), dim=-1, p=2)
            else:
                ...
                cache_dic['cache'][current['flag']][current['layer']]['v_norm'] = torch.norm(
                    rearrange(v, "(B T) H S C -> B H (T S) C", H=self.num_heads ,T=cache_dic['T'], S=cache_dic['S']), dim=-1, p=2)
