...
from ...models.cache_functions import global_force_fresh, cache_cutfresh, update_cache, force_init, score_evaluate, cal_type
...
class STDiT3Block(nn.Module):
    ...
    def forward(
        ...
        types = cal_type(cache_dic, current)
        #current['is_force_fresh'] = is_force_fresh
        
        # modulate (attention)
        current['module'] = 'attn'

        if types[current['module']] == 'full':
            ...
            if self.temporal:
                ...
                x_m = self.attn(x_m, cache_dic=cache_dic, current=current)
        ...
        elif types[current['module']] == 'ToCa':           
        ...
        if types[current['module']] == 'full':
            cache_dic['cache'][current['flag']][current['layer']][current['module']], cache_dic['cross_attn_map'][current['flag']][current['layer']] = self.cross_attn(x, y, mask, cache_dic=cache_dic)
            force_init(cache_dic, current, x)
        ...
        if (current['layer'] ==27) and types['all_aggressive']:
            x = cache_dic['cache'][current['flag']]['noise']
        if (current['layer'] ==27):
            cache_dic['cache'][current['flag']]['noise'] = x
