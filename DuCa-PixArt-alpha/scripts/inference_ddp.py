...
def get_args():
    ...
    parser.add_argument("--cache_type", type=str, choices=['random', 'attention', 'similarity', 'norm', 'kv-norm', 'compress'], default='attention')
    ...
    parser.add_argument("--test_FLOPs", action="store_true", default=False)
...
if __name__ == '__main__':
    ...
    save_root = os.path.join(img_save_dir, f"{args.dataset}_epoch{epoch_name}_step{step_name}_scale{args.cfg_scale}_step{sample_steps}_size{args.image_size}_bs{args.bs}_samp{args.sampling_algo}_seed{args.seed}")
