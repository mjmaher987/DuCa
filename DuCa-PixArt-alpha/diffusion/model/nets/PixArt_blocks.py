...
class MultiHeadCrossAttention(nn.Module):
    ...
    def forward(self, x, cond, mask=None, cache_dic=None, current=None):
        ...
        #x = self.proj(x)
        #x = self.proj_drop(x)
        if cache_dic.get('test_FLOPs', False):

            B0, N0, Head0, head_hid0 = q.shape
            B1, N1, Head1, head_hid1 = k.shape

            flops = (
                B0 * N0 * C  * C * 2 # Q projection
                + B1 * N1 * C  * C  * 2 * 2 # KV projection
                + B0 * Head0 * N0 * head_hid0  # Scale q
                + B0 * Head0 * N0 * N1
                + B0 * Head0 * N0 * N1 * head_hid0 * 2 # Q @ K
                + B0 * Head0 * N0 * N1 * 5 # Softmax
                + B0 * Head0 * N0 * N1 * head_hid0 * 2 # Attn @ V
                + B * N0 * C  * C  * 2 # Projection
            )
...
class WindowAttention(Attention_):
    ...
    def forward(self, x, mask=None, cache_dic=None, current=None):
        B, N, C = x.shape
        ...
        if cache_dic['cache_type'] == 'kv-norm':
            #cache_dic['cache'][-1][current['layer']]['k_norm'] = torch.norm(k, dim=-1, p=2)
        ...
        if cache_dic.get('test_FLOPs', False):
            flops = (
                ...
                + B * self.num_heads * N * C // self.num_heads  # Scale q
                + B * self.num_heads * N * N * C // self.num_heads * 2 # Q @ K
                + B * self.num_heads * N * N  * 5 # Softmax
                + B * self.num_heads * N * N * C // self.num_heads * 2 # Attn @ V
