...
from diffusion.model.cache_functions import global_force_fresh, cache_cutfresh, update_cache, force_init, cal_type
...
#
    ...
    def forward(self, x, y, t, current, cache_dic, mask=None, **kwargs):
        ...
        if current['type'] == 'full': # Compute all tokens, and save them to cache
            
            shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = (self.scale_shift_table[None] + t.reshape(B, 6, -1)).chunk(6, dim=1)
            
            # LayerNorm FLOPs (for both norm1 and norm2)
            if test_FLOPs:
                cache_dic['flops']+= 2 * B * N * C

            # AdaLN FLOPs (SiLU and Linear)
            if test_FLOPs:
                cache_dic['flops']+= B * C  # SiLU FLOPs
                cache_dic['flops']+= B * C * 6 * C  # Linear FLOPs in adaLN_modulation
            
            current['module'] = 'attn'
            cache_dic['cache'][-1][current['layer']][current['module']], cache_dic['attn_map'][-1][current['layer']] = self.attn(t2i_modulate(self.norm1(x), shift_msa, scale_msa), cache_dic=cache_dic, current = current)#.reshape(B, N, C)
            #cache_dic['cache'][-1][current['layer']][current['module']] = self.attn(t2i_modulate(self.norm1(x), shift_msa, scale_msa), cache_dic=cache_dic)#.reshape(B, N, C)
            ...
            cache_dic['cache'][-1][current['layer']][current['module']], cache_dic['cross_attn_map'][-1][current['layer']] = self.cross_attn(x, y, mask, cache_dic=cache_dic)
            force_init(cache_dic, current, x)
            ...
            if test_FLOPs:
                ...
                cache_dic['flops']+= B * N * C * mlp_hidden_dim * 2 # First projection
                cache_dic['flops']+= B * N * mlp_hidden_dim * C * 2 # Second projection
                cache_dic['flops']+= B * N * mlp_hidden_dim * 6 # GELU activation
            ...
            if fresh_indices.shape[1]!=0:
                fresh_tokens, fresh_cross_attn_map = self.cross_attn(fresh_tokens, y, mask, cache_dic=cache_dic)
            ...
            if test_FLOPs:
                B_fresh, N_fresh, C_fresh = fresh_tokens.shape  # 获取 fresh tokens 的 shape
                mlp_hidden_dim = int(C_fresh * 4)  # Assuming mlp_ratio = 4
                cache_dic['flops']+= B_fresh * N_fresh * C_fresh * mlp_hidden_dim * 2 # First projection
                cache_dic['flops']+= B_fresh * N_fresh * mlp_hidden_dim * C_fresh * 2 # Second projection
                cache_dic['flops']+= B_fresh * N_fresh * mlp_hidden_dim * 6 # GELU activation
        ...
        #if (current['layer'] == 27) and (current['step'] != 0) and True:
        #    cache_dic['similarity_score'] = 1 - nn.functional.cosine_similarity(x, cache_dic['cache'][-1]['noise'], dim = -1)
        #    cache_dic['similarity_score'] = nn.functional.normalize(cache_dic['similarity_score'], dim = -1, p=2)

        if current['layer'] == 27:
            cache_dic['cache'][-1]['noise'] = x
        
        if test_FLOPs and (current['layer'] == 27) and (current['step'] == current['num_steps'] -1):
...
class PixArtMS(PixArt):
    ...
    def forward(self, x, timestep, current, cache_dic, y, mask=None, data_info=None, **kwargs):
        ...
        for i, block in enumerate(self.blocks):
            current['layer'] = i
            x = auto_grad_checkpoint(block, x, y, t0, current, cache_dic, y_lens, **kwargs)  # (N, T, D) #support grad checkpoint
        x = self.final_layer(x, t)  # (N, T, patch_size ** 2 * out_channels)
        x = self.unpatchify(x)  # (N, out_channels, H, W)
        return x

    def forward_with_dpmsolver(self, x, timestep, current, cache_dic, y, data_info=None, **kwargs):
        """
        ...
        model_out = self.forward(x, timestep, current, cache_dic, y, data_info=data_info, **kwargs)
