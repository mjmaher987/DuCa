...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, c, current, cache_dic):
        ...
        if force_fresh: # Force Activation: Compute all tokens and save them in cache
            ...
            current['module'] = 'attn' 
            # no extra computation for final version, you can add the following line to do some partial computation, but always worse results caused by error propagation mentioned in toca.
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, c, current, cache_dic):
        ...
        if current['type'] == 'full':  # Force Activation: Compute all tokens and save them in cache
            ...
            current['module'] = 'attn'
            x = x + gate_msa.unsqueeze(1) * cache_dic['cache'][-1][layer]['attn']
            #x = x + cache_dic['cache'][-1][layer]['attn']
        ...
        elif current['type'] == 'Residual':

            if current['layer'] == cache_dic['residual_start']:
                cache_dic['cache'][-1]['x_base'] = x
            
            if current['layer'] in range(cache_dic['residual_start'], cache_dic['residual_end'] + 1):
                pass
...
class DiT(nn.Module):
    ...
    #def forward_with_cfg(self, x, t, y, cfg_scale):
        ...
        half_eps = eps_guide + cfg_scale * (eps_original - eps_guide)
